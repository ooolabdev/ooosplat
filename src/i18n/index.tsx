...
const zhCN = {
  ...
  "quality.label": "生成质量",
  "quality.fast": "快速",
  "quality.fastHint": "速度优先 · 目标约 6 FPS，必要时最多 9 FPS",
  "quality.balanced": "均衡",
  "quality.balancedHint": "默认推荐 · 目标约 8 FPS，必要时最多 12 FPS",
  "quality.high": "精细",
  "quality.highHint": "质量优先 · 目标约 10 FPS，必要时最多 15 FPS",
  ...
  "alpha.imagesTitle": "检测到 Alpha 通道",
  "alpha.videoTitle": "检测到 Alpha 通道",
  "alpha.imagesHint": "生成时将精确检测透明度，并按需生成 COLMAP Mask",
  ...
  "generate.start": "开始生成",
  "planner.beta": "BETA",
  "planner.title": "自动重建 Planner",
  "planner.description": "OOOSplat 可以自动分析拍摄素材，在当前质量档位允许的范围内调整抽帧、匹配和重建策略，并在重建出现问题时自动尝试提高成功率。Beta 版本仍在测试中，特殊素材也可以继续使用传统流程。",
  "planner.remember": "以后生成时始终开启",
  "planner.traditional": "使用传统流程",
  "planner.enable": "开启 Beta",
  "planner.settingsTitle": "Auto Reconstruction Planner",
  "planner.settingsDescription": "选择开始生成时是否询问。传统流程选择只对当次任务生效，不会保存为永久关闭。",
  "planner.askEachTime": "每次生成时询问",
  "planner.askEachTimeHint": "每次开始前选择传统流程或 Planner Beta。",
  "planner.alwaysOn": "始终开启",
  "planner.alwaysOnHint": "新任务默认直接使用 Planner Beta；可随时改回询问。",
  "planner.privacy": "隐私设置",
  "progress.title": "实时进程",
...
const en: Record<TranslationKey, string> = {
  ...
  "planner.beta": "BETA", "planner.title": "Auto Reconstruction Planner", "planner.description": "OOOSplat can analyze capture media and adapt frame selection, matching, and reconstruction within the selected quality budget. If reconstruction is weak, it can run bounded recovery attempts to improve success. This Beta is still being tested, and the traditional pipeline remains available for unusual media.", "planner.remember": "Always enable for future generations", "planner.traditional": "Use traditional pipeline", "planner.enable": "Enable Beta", "planner.settingsTitle": "Auto Reconstruction Planner", "planner.settingsDescription": "Choose whether OOOSplat asks before generation. Choosing the traditional pipeline only affects that run and is never saved as Always Off.", "planner.askEachTime": "Ask each time", "planner.askEachTimeHint": "Choose the traditional pipeline or Planner Beta before each new task.", "planner.alwaysOn": "Always on", "planner.alwaysOnHint": "Use Planner Beta for new tasks without prompting; you can switch back at any time.", "planner.privacy": "Privacy settings",
  ...
  "quality.label": "Generation quality", "quality.fast": "Fast", "quality.fastHint": "Speed first · targets about 6 FPS, up to 9 when needed", "quality.balanced": "Balanced", "quality.balancedHint": "Recommended · targets about 8 FPS, up to 12 when needed", "quality.high": "Detailed", "quality.highHint": "Quality first · targets about 10 FPS, up to 15 when needed",
  ...
  "alpha.imagesTitle": "Alpha channel detected", "alpha.videoTitle": "Alpha channel detected", "alpha.imagesHint": "Transparency will be checked precisely during generation and COLMAP masks created only when needed", "alpha.videoHint": "Transparent frames and COLMAP masks will be extracted automatically · {format}",
...
const exactPipelineEnglish: Record<string, string> = {
  ...
  "正在恢复已保存的画面计划": "Restoring the saved frame plan",
  "Planner 正在根据质量预算选择画面": "Planner is selecting frames within the quality budget",
  ...
  "正在快速读取图片头": "Reading image headers",
  "正在准备原始图片并生成 COLMAP Alpha Mask": "Preparing source images and COLMAP alpha masks",
  "正在准备图片序列": "Preparing image sequence",
  "正在建立画面链接": "Linking image frames",
  "正在建立画面链接并检查 Alpha 通道": "Linking image frames and checking alpha channels",
  "正在校验图片与 Mask": "Validating images and masks",
  "图片与 Mask 完整性校验完成": "Image and mask validation completed",
...
export function localizePipelineMessage(locale: Locale, message: string): string {
  ...
  const patterns: Array<[RegExp, (...values: string[]) => string]> = [
    ...
    [/^正在导入图片 ([\d,]+)\/([\d,]+) 张$/, (current, total) => `Importing images ${current}/${total}`],
    [/^正在建立画面链接或复制 ([\d,]+)\/([\d,]+) 张$/, (current, total) => `Linking or copying frames ${current}/${total}`],
    [/^正在检测 Alpha 并生成 Mask ([\d,]+)\/([\d,]+) 张$/, (current, total) => `Checking alpha and generating masks ${current}/${total}`],
    [/^正在补全不透明 Mask ([\d,]+)\/([\d,]+) 张$/, (current, total) => `Completing opaque masks ${current}/${total}`],
    ...
    [/^图片序列 ([\d,]+) 张 · ([\d]+)×([\d]+)( · 检测到 Alpha 通道)?$/, (count, width, height, alpha) => `Image sequence ${count} images · ${width}×${height}${alpha ? " · alpha channel detected" : ""}`],
